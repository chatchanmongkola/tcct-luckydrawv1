...
export function DashboardShell({ user, children }: DashboardShellProps) {
  ...
  return (
    <div className="min-h-screen bg-slate-100">
      <header className="sticky top-0 z-30 h-16 border-b border-slate-200 bg-white/95 backdrop-blur-sm">
        <div className="mx-auto flex h-full max-w-7xl items-center justify-between px-4 sm:px-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs uppercase tracking-[0.2em] text-slate-400">TCCtech Event</p>
              <p className="text-sm font-semibold text-slate-900">Lucky Draw</p>
            </div>
          </div>
      ...
      <DashboardSidebar user={user} isOpen={isOpen} onClose={() => setIsOpen(false)} />
